    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($user) {
                ...
                $delete = '<form action="' . route('admin.translations.destroy', $user->id) . '" method="POST" class="delete-form" style="display:inline-block">
                    <input type="hidden" name="_token" value="' . csrf_token() . '">
                    <input type="hidden" name="_method" value="DELETE">
                    <button type="submit" class="btn btn-sm fs-5 text-danger delete-btn" onclick="return confirm(\'Silmək istədiyinizə əminsiniz?\')">
