...
class OurStudioGalleryDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', fn($row) => view('backend.pages.our_studio_galleries.action', compact('row'))->render())
            ->editColumn('image', fn($row) => '<img src="/storage/' . $row->image . '" width="80" class="img-thumbnail" />')
            ->rawColumns(['action', 'image'])
            ->setRowId('id');
    }

    public function query(OurStudioGallery $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('our_studio_galleries-table')
    ...
    {
        return 'our_studio_galleries_' . date('YmdHis');
    }
}
