<div class="dropdown">
    ...
    <ul class="dropdown-menu" aria-labelledby="portfolioActionDropdown{{ $row->id }}">
        {{-- Redaktə --}}
        <li>
            <a class="dropdown-item" href="{{ route('admin.social-link.edit', ['serviceCategory' => $row->id]) }}">
        ...
        @if(isset($row->is_active))
            <li>
                <form action="{{ route('admin.social-link.update', ['serviceCategory' => $row->id]) }}" method="POST" class="d-inline">
                    @csrf
                    @method('PATCH')
        ...
        {{-- Sil --}}
        <li>
            <form action="{{ route('admin.social-link.destroy', ['serviceCategory' => $row->id]) }}" method="POST"
